    ...
    public final void run() {
        ...
        try {
            ...
            try {
                ...
                result = Result.m97constructorimpl(Unit.INSTANCE);
            } catch (Throwable th) {
                th = th;
                Result.Companion companion2 = Result.Companion;
                result = Result.m97constructorimpl(ResultKt.createFailure(th));
                handleFatalException(fatalException, Result.m100exceptionOrNullimpl(result));
            }
        }
        try {
            ...
            try {
                if (job != null && !job.isActive()) {
                    ...
                    if (DebugKt.getRECOVER_STACK_TRACES()) {
                        try {
                            if (continuation instanceof CoroutineStackFrame) {
                                ...
                                continuation.resumeWith(Result.m97constructorimpl(ResultKt.createFailure(cancellationException)));
                            }
                        } catch (Throwable th3) {
                            th = th3;
                    ...
                    continuation.resumeWith(Result.m97constructorimpl(ResultKt.createFailure(exception)));
                } else {
                    T successfulResult$kotlinx_coroutines_core = getSuccessfulResult$kotlinx_coroutines_core(state);
                    Result.Companion companion5 = Result.Companion;
                    continuation.resumeWith(Result.m97constructorimpl(successfulResult$kotlinx_coroutines_core));
                }
